...
Of course, we want to deploy it to Shuttle, so let's start with a new Shuttle project. If this is your first time using Shuttle, you can follow the [installation guide](https://docs.shuttle.dev/introduction/installation), or check out the previous issue in our [archive](https://www.shuttle.dev/launchpad/).
