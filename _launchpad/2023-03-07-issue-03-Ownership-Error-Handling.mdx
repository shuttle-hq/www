...
Welcome to the third issue of Shuttle Launchpad! Before we go into the nitty gritty details of today's issue, let's take some time to celebrate the Shuttle reaching beta status! 🎉 Check out all our announcements on [shuttle.rs/beta](https://shuttle.rs/beta).

Btw... we're [growing our team](https://www.shuttle.rs/about#careers)!
...
Of course, we want to deploy it to Shuttle, so let's start with a new Shuttle project. If this is your first time using Shuttle, you can follow the [installation guide](https://docs.shuttle.rs/introduction/installation), or check out the previous issue in our [archive](https://shuttle.rs/launchpad/).
