...
Quick announcement: we are hosting a **Christmas Code Hunt**! It's a 16-day long event inspired by the Advent of Code! Check it out here: [shuttle.dev/cch](https://www.shuttle.dev/cch). Now, without further ado;
...
- If you create a `SharedState` trait instead of a concrete type, you can easily swap out the state for a different implementation. For example, you could use a `HashMap` in your tests, but a `Redis` instance in production. Or even better, use [Shuttle Perist](https://docs.shuttle.dev/resources/shuttle-persist).
