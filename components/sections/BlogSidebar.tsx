					...
					<div className='flex flex-wrap gap-2'>
						{(tags || []).map((tag: string) => {
							return (
								<Link
									key={tag}
									className='border-gradient-h relative rounded-full px-2 py-1 text-sm outline outline-1 outline-black/10 transition-all duration-500 hover:outline-transparent hover:after:rounded-full hover:after:bg-gradient-to-r hover:after:from-[#FC540C] hover:after:to-[#FFD76F] dark:bg-black dark:outline-[#1E1B19] dark:hover:text-white'
									href={`/blog/tags/${tag}`}
								>
									{tag}
								</Link>
					...
					<div className='rounded-3xl border border-black/10 py-4 px-6 dark:border-white/10'>
						...
						<div className='relative flex flex-col gap-2 space-y-3'>
							...
							<div className='mt-2'>
								<Link
									href='/blog/tags/all'
									className='group flex cursor-pointer items-center space-x-2 text-sm text-slate-500 hover:text-slate-900 hover:underline dark:text-[#7A7A7A] hover:dark:text-white'
								>
									<span>View all</span>
									<svg
										width='17'
										...
										fill='none'
										xmlns='http://www.w3.org/2000/svg'
										className='relative left-0 transition-all duration-500 group-hover:left-2'
									>
										<path
