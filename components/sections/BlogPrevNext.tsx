...
const BlogPrevNext: FC<BlogPrevNextProps> = ({ prevPost, nextPost }) => {
	return (
		<div className='grid grid-cols-1 gap-5 overflow-visible py-7 lg:grid-cols-2 xl:gap-12'>
			<div>{prevPost && <DirectionCard post={prevPost} hoverDirection='left' />}</div>
			<div>
				{nextPost && <DirectionCard post={nextPost} className='text-right' hoverDirection='right' />}
			</div>
		</div>
	)
}
...
const DirectionCard: FC<DirectionCardProps> = ({ post, className, hoverDirection }) => {
	return (
		<Link
			href={`/blog/${post.url}`}
			className={clsx(
