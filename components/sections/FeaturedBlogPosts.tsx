        ...
        <SplideTrack className="!overflow-visible">
          {posts.map((post, index) => (
            <Article
              ...
              post={post}
              className={
                index === 0 ? "lg:pt-10" : index === 1 ? "lg:pt-5" : ""
              }
            />
          ))}
        </SplideTrack>
...
const Article: FC<ArticleProps> = ({ post, className }) => {
  ...
  const [authorImage, setAuthorImage] = useState<string>(
    firstAuthor?.author_image_url || "/images/logo.png",
  );
  ...
  return (
    <SplideSlide className={className}>
      <Link
        ...
        eventName={`homepage_whatsnew_blogpost_${post.title}`}
      >
        ...
        <div className="p-6">
          <h1 className="font-gradual text-xl font-bold text-white">
            {post.title}
          </h1>
          ...
          <div className="mt-6 flex items-center gap-2.5">
            <Image
              ...
              height={40}
              className={
                authorImage !== "/images/logo.png" ? "rounded-full" : ""
              }
            />
            <div>
