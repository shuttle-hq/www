...
```
Our free database offering is via a shared database server (with users having separate databases for each application). However, we are now offering 100% isolated AWS RDS databases as a paid add-on which you can find more about [here](https://www.shuttle.rs/pricing) which supports MySQL, Postgres and MariaDB.
