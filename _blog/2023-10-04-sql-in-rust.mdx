...
- It's compatible with all your favourite flavours of SQL (MySQL, SQLite, Postgres, MSSQL)
...
listener.listen("chan0").await.unwrap();
...
loop {
  sqlx::query("SELECT pg_notify('chan0', 'Hello world!')").execute(&pool).await;
}
```
...
```
Our free database offering is via a shared database server (with users having separate databases for each application). However, we are now offering 100% isolated AWS RDS databases with our Pro tier which you can find more about [here](https://www.shuttle.rs/pricing) which supports MySQL, Postgres and MariaDB. 
