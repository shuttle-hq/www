...
We can also use tuple-like struct syntax and named-field struct syntax for enum variants within our enum; this is because unlike in other OOP languages, Rust enums are actually **sum types**. You can read more about how powerful Rust enums are in another article we wrote [here.](https://shuttle.dev/blog/2023/11/23/enums-in-rust) You can have optional arguments by simply wrapping the types in `Option`, but if you want to add a flag to a command you can use `bool`, since [`clap`](https://github.com/clap-rs/clap) recognises that flags are either there or not there. Let's have a look at what this might look like:
```rust
use clap::{Parser, Subcommand};
...
Interested in more?
- Check out how you can use raw SQL in Rust with SQLx [here.](https://shuttle.dev/blog/2023/10/04/sql-in-rust)
- If you'd like to know more about how macros work, [this article should help you out.](https://shuttle.dev/blog/2022/12/23/procedural-macros)
