...
We _could_ use `fetch_all()`, but by using `fetch()` we get a `Stream` back and can skip dealing with `RowNotFound` SQL errors by checking whether or not there are any rows to fetch.
...
As you can see below it is _mostly_ the same as grabbing data for all websites, except now we also have last month's data (split by day) to add to our return results:
