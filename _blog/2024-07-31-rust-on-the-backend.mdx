...
Below is an example of adding a database pool from `sqlx` to shared application state and using it within a router, complete with a basic CRUD route for grabbing all users from a database table and returning it as a JSON response:
```rust
use sqlx::PgPool;
...
While frontloading so much to the compiler does of course increase compilation times, your application will typically be running for much longer in production. Compilation times can additionally be significantly reduced with the help of [crane](https://github.com/ipetkov/crane) and [mold linker](https://github.com/rui314/mold), easing the developer experience while iterating early on. Although first-time compiles can take a few minutes, typically with subsequent compilations it takes much less thereafter.

### Concurrency
Concurrency has always been a difficult problem to solve. This can be made more difficult with language design choices designed to abstract away complexity. For example Python's Global Interpreter Lock (GIL) has undeniably had benefits in terms of development and making it easier to run C extensions in Python. However, it can also significantly increase the complexity of multithreaded applications through lock acqusition and thread contention, as well as making it more difficult to express parallelism.

In contrast, Rust doesn't hide the complexity of writing concurrent data structures and general concurrent programs. This can make it somewhat intimidating to get started. However, libraries have been created that significantly build on the stdlib primitives to provide much more usable abstractions that make handling concurrency much easier at scale. For example: [dashmap](https://docs.rs/dashmap/latest/dashmap/), a concurrent `HashMap` implementation, as well as [parking-lot](https://github.com/Amanieu/parking_lot) which primarily provides the parking and unparking of threads but provides deadlock detection for mutexes and other synchronization primitives.
...
Read more:
- [Getting started with Actix Web](https://www.shuttle.rs/blog/2023/12/15/using-actix-rust)
