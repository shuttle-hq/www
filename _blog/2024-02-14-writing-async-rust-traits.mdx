...
description: This article explores how you can write async Rust manually using the `std::future::Future` trait, as well as async traits.
author: josh
...
Async in Rust enables concurrent execution of tasks by using `futures` - an async primitive provided by the standard Rust library. We also have things like `channels`, `TcpStreams`, `TcpListener`, `Mutex`es, and other types that enable concurrency. From there, it’s left up to the user to implement their own async functions. From this, popular crates like `tokio` have been created which form the backbone of async Rust. Additionally, features like greenthreading (virtual threading), as well as native CPU threading, can be used for concurrency!
...
So far, so good. We can see here that a `std::instant::Instant` gets generated and a `Delay` is created. We then instantiate a `MainFuture` that is then polled. 

## Async traits in Rust

Now it's time to move onto the big boy toys: async Rust traits! There's a few ways to do them, which we will talk about below. But first, we'll look at an example that showcases how you can write an async Rust trait manually.
...
First, let’s have a look at the `Service` trait from the `tower` crate - a well-known crate in the Rust async web ecosystem:
...
What about if we don’t know what the future looks like or want to allow the user to decide on the final function signature? For example, if they want to return a box-pinned future? Or just a regular future? We can’t use `async fn` here. In this case, we use `impl Future` as the return type:
...
As you’ve seen, there are a lot of cool different ways you can write async Rust traits! You can do it either the low-level or high-level way, or mix them.
