...
This has led to many frameworks attempting to emulate the Rails philosophy - Loco.rs being no different. In this case, however, it aims to solve a long-standing issue within the Rust web backend framework in terms of there being no truly "batteries-included" framework. Let's talk about it.
...
## Getting Started
To get started with Loco, you need to use their CLI which you can install by using the following:
```bash
cargo install loco-cli
```

You can start a new project by using `loco new` - it'll ask you what the name of your app is and then what kind of app you want. For this article, we'll be talking about the full SaaS starter application.

## Routing in Loco
Although Loco uses axum under the hood, it abstracts some things away into config files that you can find in the `configuration` folder. 
...
Once you've finished adding all the controllers and routes you want, you can use `cargo run routes` to display all of the routes your application currently has.

## Migrations
Handling migrations in Loco is quite easy through the CLI - to generate a migration you can just use `cargo run generate migration test`, which generates a migration for the `test` table. The files generated can be found in `migrations` (in the project root) and use `sea_orm`, which is an ORM that builds off of `sqlx` - the most, one of the most popular async-based SQL libraries.

When you generate your migrations and go to the file itself, it should look something like this:
```rust
...
impl MigrationTrait for Migration {
    async fn up(&self, manager: &SchemaManager) -> Result<(), DbErr> {
        manager
            .create_table(
                table_auto(Tests::Table)
                    .col(pk_auto(Tests::Id).borrow_mut())
                    .to_owned(),
            )
            .await
    }
...
```
From this file, you can add more columns as required - make sure to reflect this in the enum at the bottom! If you are stuck on how to expand your migrations, you can always check the base migration files or you can check out the [`sea-orm` documentation.](https://www.sea-ql.org/SeaORM/docs/index/)

When you're done adding migrations, all you need to do is use `cargo run db migrate` and the CLI will do all of the heavy lifting for you - nothing else is needed!

## Models
Now that you've generated a table migration, you probably want to generate a model that your application can use. Once you've generated the migration, you'll want to run the following:
```bash
cargo run generate model <name-of-migration>
```
This will then generate a model that looks similar to your migration. Note that if you want to initialise with extra fields, you will want to check the [reference docs](https://loco.rs/docs/the-app/models/) so you can find what fields you need to use.

Although it comes with an initially blank file, you will probably find something that looks like this:
```rust
...
## Job Processing
Like with everything else in Loco, you can also generate workers and tasks via the CLI. Running `cargo run generate task` or `cargo run generate worker` will let you generate a task or a worker at will.
...
## Deployment
Currently, Loco.rs allows you to generate a deployment by using `cargo run generate deployment` - which lets you choose between Docker and Shuttle. When choosing Docker it will generate a Dockerfile that you can use to deploy anywhere, but when you pick Shuttle it will automatically generate everything you need for a Shuttle deployment - no further work required! You can then use the Shuttle CLI to start a new project and deploy it.
