...
When we run our app we'll run the migrate macro, which will automatically attempt to run our migrations and add a new migration entry to the table so it won't automatically try to run the migration again. However, if you change your migrations file it'll return an invalid migrations error, which means you'll have to visit your database using psql or your favourite DB management application and delete all of the tables in your database.
...
cargo shuttle init
```
For the purposes of this project we'll be initialising the backend within our frontend and initialising the directory as 
`backend`. 
...
// we are required to implement FromRef as it will allow axum's `Key` to be cloned
// this means we can use a private cookie jar
...
Once we allow the application to use our user's credentials, Google will then fire a GET request to our chosen OAuth redirect URI as seen in the homepage router, with some URI query parameters. Although there's multiple parameters returned, for us we only need the code response given back by Google so we can exchange it for an access token. We can make a struct to extract the
query parameters:
```rs
...
Because we requested for OpenID privileges earlier, we can now access any of Google's OpenID using the access token that was 
given to us. Thankfully, this is pretty simple to do without the oauth2 crate and we only need to use a simple `Reqwest` client
with bearer auth to get the user profile data, like so:
```rs
// place this struct declaration outside of the function
...
// and passed to the AppState. Creating the client itself takes 300-500ms, saving time
...
for our purposes currently we only need the email for the purposes of verification. 

## Using OAuth with Axum Extensions
Now that we've got our access token, all we need to do is primarily is storing the token somewhere and allowing our service to
find the token. We can do this with SQLx and usage of the `PrivateCookieJar` type from `axum_extra`, which uses cryptographically secure cookies. Let's have a look at what the code would look like:
