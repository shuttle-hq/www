...
```bash
$ docker run -d -p 5432:5432 -e POSTGRES_USER=loco -e POSTGRES_DB=example-app_development -e POSTGRES_PASSWORD="loco" postgres:15.3-alpine
```
...
We are planning to release a native database integration with Loco! Stay tuned 

## Finishing Up
Thanks for reading! Loco is an awesome framework to get you started on the path to productivity.

Interested in more?
- Read about our getting started with Axum article [here](https://www.shuttle.rs/blog/2023/12/06/using-axum-rust)
- Learn about how you can implement OAuth for your application [here](https://www.shuttle.rs/blog/2023/08/30/using-oauth-with-axum)
