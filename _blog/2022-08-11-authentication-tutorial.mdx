...
With hashing, if someone gets the value in the password field they cannot find out the actual password value. The only thing this value allows is whether a plain text password matches this value. And with [salting](<https://en.wikipedia.org/wiki/Salt_(cryptography)>) different names are encoded differently. Here all these passwords were registered as _"password"_, but they have different values in the database because of salting.
...
Great so now we have a token/identifier for a _session_. Now we can use this as a key to get information about users.
