...
I tested it across a few key areas: executing tasks of varying complexity, searching codebases and documentation, and planning. These are common software engineering tasks that developers face daily. I'll walk through what worked and what didn't.
...
Gave it a broad instruction and let it choose whatever framework it thinks best. It chose Axum _obviously_ as every model seems to be using it these days.
...
I tested this with a real GitHub issue from the [shuttle-hq/shuttle](https://github.com/shuttle-hq/shuttle) repository. The task was to add a `--quiet` flag to suppress non-error output from the `shuttle run` command. I wanted to see how fast the AI can find all the relevant files in the codebase.
...
The AI scanned the entire codebase and returned results in 15 seconds:
...
Creating plans is a common process when developing software. Let's see if it can take those search results and create an implementation plan? I started a fresh chat and fed it the issue description along with all the file paths and locations it had just found.
...
The AI generated a well-structured plan:
...
The structure looks solid, but the real test is execution and I don't really know if the approach is actually good or not. So, let's see if it can complete the task.

## Complex Tasks: Implementing the Solution

Opened a new chat and fed the AI model the implementation steps and asked it to implement the changes. It completed the task in 40 seconds, making changes across multiple files.
...
After testing this AI coding model across different scenarios, here's what I think about it:

**Codebase search:** Excellent. This is where the model really excels. Fifteen seconds to scan an entire repository and find every relevant file with specific line numbers is incredibly useful.
