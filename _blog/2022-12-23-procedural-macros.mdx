...
struct X {}
```
This article will cover the concept of macros and some interesting use cases, and you certainly don't need to be a Rut expert to follow along. However, the example section assumes you have written *some* Rust (`if let`, `struct` , `trait` etc).
...
The `arrayFromString` function assumes that the caller has passed a standard array where every object has the same type. `Reflect.get` will fail at runtime as if the property doesn't exist. The downside of this is that both the dynamic property lookup and null check is expensive at runtime.

One way to catch property errors ahead of time is to us a type system on top of JavaScript like TypeScript.
...
impl BinarySerializable for u64 {
    ...
    fn deserialize<I: Iterator<Item = u8>>(iter: &mut I) -> Self {
        ...
        // @ben this won't work right? You're pulling a single byte? Don't you need 8 bytes?
...
// @ben add the `$cargo expand` prompt alongside the generated code and make sure the next is a little smaller.
...
Alternatively, adding `eprintln!("{}", expanded)` to the end of your macro code and then running `cargo check` also works for debugging. The output is not as readable but it works for malformed token streams for malformed token streams.
...
// @ben not sure what you're saying here - consider rephrasing
...
### syn-helpers
[syn-helpers](https://github.com/kaleidawave/syn-helpers) is a framework I have been working to abstract common derive patterns. Our short example works great for our `Country` struct, our macros should work across enums, generics, etc. It's still a work in progress but not all proc macros need long and complex implementations.

### Inner annotations
Sometimes we want to have some custom behaviour for a field. Rust allows for writing attributes in lots of places. The attribute can be read from the syntax tree that syn parses and are simple to lookup. They can contain token stream arguments if even more information needs to be given to the macro.
