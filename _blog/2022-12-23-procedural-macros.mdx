...
This article will cover the concept of macros and some interesting use cases, and you certainly don't need to be a Rust expert to follow along. However, the example section assumes you have written _some_ Rust (`if let` , `struct`, `trait` etc).
...
Here with `Object.entries` we can inspect the _shape_ of an object at runtime. Using the `typeof` operator we can get the type of the value. (Again assuming that all objects have the same type):
