...
Either way we want to add middleware which _injects_ the following the struct into current request.
...
_this is a bit ad hoc parsing, proper parsing should account for multiple cookies etc and could be neater 😆_.

At the end we do two **important things**. First we _extend_ the request with this lazy auth state: `req.extensions_mut().insert(...)`. Secondly we run the rest of the request stack: `next.run(req).await`.
