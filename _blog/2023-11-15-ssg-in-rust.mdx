...
author: josh
tags: [rust, testing, rust for javascript developers, guide]
thumb: testing-in-rust-thumb.png 
cover: testing-in-rust-thumb.png
date: '2023-11-08T14:30:00'
---
So it's about that time where I want to re-do my personal portfolio for the Nth time already. I don't want to use React, or Next, or whatever people call those JS frameworks. I want to build it with the language I've been using for quite a while now: Rust - and I want to prove that I can do it quickly. From my own experiences, Rust has always been seen from the outside as a language that's "overkill" and takes too much time to write anything in because of types/function signatures. While that might be true for more things involving more advanced type machinery (implementing your own job queues, implementing async traits manually, lifetimes), for a lot of things that don't require things that amount to mental space rocketry you can knock out a project surprisingly quickly. 

We'll be making a website in the form of a static site generator: if you want to see the final result so you can deploy it straight to Shuttle, you can follow the instructions below:
1) Run `cargo shuttle init --from joshua-mo-143/shuttle-ssg` and follow the prompt
2) `cd` to the folder
3) Run `cargo shuttle deploy --allow-dirty` and you're done!

Don't forget to make sure that `cargo-shuttle` is installed and that you're logged in!

## 59:59 - Getting Started
It's time to lock in. I went on Youtube and put on a breakcore playlist, then used `cargo shuttle init --template axum` to quickly spin up what I wanted. I used the project name `ssg` and put it in my projects folder, then got to work. If you want to deploy a project in the fastest time possible with no time lag between finishing a project and deploying it, Shuttle's definitely the way to go - you don't need a Dockerfile or any config files, you just use their runtime, provision the things you need and go.

I created a `Shuttle.toml` file and added the following:
```rust
// Shuttle.toml
...
async fn hello_world() -> impl IntoResponse {
    ...
    let mut html_output = get_page_header(string_output, &state.domain, state.filenames);
...
At this point, I panicked a little bit and had a quick look at how to iterate through files in a directory with Tokio. I rememebered that it returned a stream - so you could use `while let Some...` to retrieve a stream of files and then append them all to a vector. Great!
...
```
I decided for now to only add pages in the main pages directory - the rest can come later. Now that the filename vector is in the app state, we can add it as an argument to our function for creating the header, then iterate through it and push anchor tags for each route.

```rust
// src/main.rs
fn get_page_header(input: &str, domain: &str, links: Vec<String>) -> String {
// .. rest of code
...
```
Next, I wanted to add the Open Graph tag for URL (`og:url`). Thankfully, Shuttle has a helper crate for this thing exactly called `shuttle-metadata` that gives you all the information about your project! I `cargo add`ed it, then added it into my main function and added a variable that forms the domain string based on whether we're in debug or release mode:
```rust
// src/main.rs
#[shuttle_runtime::main]
async fn main(
