...
description: Looking at what observability is, how it can help you and how to use it with Rust
author: josh
tags: [rust, observability]
thumb: what-is-observability-rust-thumb.webp
cover: what-is-observability-rust-thumb.webp
...
We're excited to announce that **starting** **this Wednesday**, all users will have access to our new observability features, allowing you to send logs and traces to your preferred provider (that supports OTel)! Free users will have access to basic metrics like network IO, disk IO, CPU usage and memory usage while Pro (and above) tier users will additionally get access to log exports (via `log` and `tracing` crates), all project metrics and all project traces.

Our Shuttle runtime takes care of the whole process for you end-to-end, allowing you to simply use your API key with a relevant supported provider and it’ll **just** work. Our initial launch will support BetterStack integration, with plans to expand to other provide other telemetry export destinations.

What observability platform would you like to see supported next? [Open an issue in our Github repo now.](https://github.com/shuttle-hq/shuttle)
