---
title: Using Reqwest to Write a Web Scraper in Rust
...
# Using Reqwest to Write a Web Scraper in Rust
...
In this guide to web scraping in Rust, we will write a Rust web scraper that will scrape Amazon for Raspberry Pi products and get their prices, then store them in a PostgresQL database for further processing.
...
## Getting Started
Let's make a new project by using `cargo shuttle init`. For this project we'll simply call it `webscraper` - you'll want the `Custom` web service, which will spawn a new Cargo project with `shuttle-runtime` added.
...
```
We'll also want to install `sqlx-cli`, which is a useful tool for managing our SQL migrations. We can install it by running te following:
```bash
...
```
If the HTTP request was successful, we will then need to change `retry_attempts` back to 0.

Assuming the HTTP request is successful, we'll get a HTML body that we can parse using `scraper`. We'll want to make sure we prepare our HTML by parsing it as a HTML fragment, and then declaring our initial `scraper::Selector`:
```rust
