...
A while ago, I wrote a [post](https://robertohuertas.com/2023/01/09/shuttle-rust-backend-deployment/) about [Shuttle](https://shuttle.dev/), where I explained how ridiculously easy it is to deploy a Rust backend to the cloud by using their [CLI tool](https://docs.shuttle.rs/introduction/quick-start).

[Shuttle](https://shuttle.dev/) is still in beta, and although its observability features are not really polished yet, they offer [support](https://docs.shuttle.rs/introduction/telemetry) for [Tokio Tracing](https://docs.rs/tracing/latest/tracing/) and a way to [view logs](https://docs.shuttle.rs/introduction/telemetry#viewing-logs) by using their CLI tool.
...
If we could do something like that with [Shuttle](https://shuttle.dev/), it would be great. But, as we just mentioned, in the case of [Shuttle](https://shuttle.dev/), we **don't have access to the underlying infrastructure**, so we need to find a way to send our logs to [Datadog](https://datadoghq.com) from our application.
...
But for that, we'll need to get **access to the subscriber instance prior to its initialization**, and it turns out that [Shuttle](https://shuttle.dev/) provides a way to do that just by disabling the default features on `shuttle-runtime` crate.
...
As a walkthrough, we are going to create a new [Shuttle](https://shuttle.dev/) project from scratch.
...
First of all, we need to create a new [Shuttle](https://shuttle.dev/) project. You can do that by using the [Shuttle CLI](https://docs.shuttle.rs/getting-started/cli):
...
As you can see, it's pretty easy to send your logs to [Datadog](https://datadoghq.com) from your [Shuttle](https://shuttle.dev/) powered backend.
