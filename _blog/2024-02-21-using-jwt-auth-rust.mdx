...
description: "This article explores implementing authentication in a Rust API using JSON Web Tokens (JWTs)."
author: josh
...
To start with, we’ll need to create a decoding and encoding key that can take a `&[u8]` (u8 slice):
...
When we run this, it should output some JSON containing the JWT and type of token - which should be the `Bearer` type. You can prove that this should be a Bearer token because in `FromRequestParts` we extract from the `Authorization: Bearer ..` header.
...
- Read more about using SQL with SQLx [here](https://www.shuttle.rs/blog/2023/10/04/sql-in-rust).
- Read more about session token based authentication [here](https://www.shuttle.rs/blog/2022/08/11/authentication-tutorial).
