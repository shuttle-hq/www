---
title: "Building a Web Analytics Tool with JS, htmx and Rust" 
description: This article documents how you can build a simple privacy-respecting analytics tool using a JavaScript script and handle the backend with Rust. 
author: josh
...
htmx is a lightweight JavaScript library for dynamic web development, enabling real-time content updates without full-page reloads. 

This time, we're building a privacy-respecting analytics tool which uses htmx, Tera and Axum with Shuttle.
...
## Getting Started
You'll need to use `cargo shuttle init --template axum` to get started (if you don't have it installed, you can use `cargo install cargo-shuttle` then log in using `cargo shuttle login` to get started!). Once you're done, you'll want to use the following one-liner to install your dependencies:

```bash
cargo add axum chrono serde serde-json shuttle-metadata shuttle-shared-db sqlx tera tokio url -F \
axum/headers,axum/macros,chrono/clock,chrono/serde,shuttle-shared-db/postgres,sqlx/runtime-tokio-native-tls,\
sqlx/postgres,sqlx/chrono,url/serde
```
...
async fn main(
    ...
    let domain = if cfg!(debug_assertions) {
        ...
        format!("https://{}.shuttleapp.rs", metadata.project_name.to_string())
    };
...
Now we can set up a function handler for returning the CSS body that our webpages can retrieve:
```rust
// src/frontend.rs
...
## Writing Our Analytics Script

For this part,  we'll need to engage in writing some JavaScript. For this part, it is not too complex: we use search parameters to hold a randomly-generated, anonymous user ID and if the referrer from the last page has the same domain host, use the same UUID - otherwise, generate a new one. The UUID is necessary so that we can distinguish user sessions from each other.

We then replace the last page history (the current page) with the flat URL so the user doesn't see the analytics tracking parameters, as well as taking note of what the current time is. Once the page loads, we then add the UUID to every anchor link where the page has the same host as the current webpage. We also add an event trigger on page visibility change to Show so that the page will automatically send a HTTP request to our analytics endpoint.
...
pub async fn query_uri(
    ...
    let mut meme = format!("{}{}{}", query.domain.host_str().unwrap(), 
             if query.domain.port().is_some() {":"} else {},
    if query.domain.port().is_some() {query.domain.port().unwrap()} else {}
    );

    let mut ctx = Context::new();
    ctx.insert("data", &data);
    ctx.insert("domain", &query.domain.clone().into_string());
    ctx.insert("domain_base", &meme);
...
## Deployment
Now that we're done, all you need to do is `cargo shuttle deploy` (with the `--ad` flag if on a dirty Git branch) and you're up and away!

## Finishing Up
Thanks for reading! I hope this has given some insight into how you can incorporate htmx in applications.
