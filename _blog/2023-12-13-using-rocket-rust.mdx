...
### Error Handling
Instead of implementing a trait for your errors, error handling is done a bit differently in Rocket. Instead of implementing an error handling trait, you handle errors by using an error handler which is called a "catcher". You can use a handler function for creating a catcher, like so:
```rust
#[catch(default)]
