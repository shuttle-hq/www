Rocket is awesome! (todo: fill this paragraph out)

We'll be talking primarily about how you can get started with Rocket 0.5, as well as how you can migrate to it if you're on an earlier version.
...
## Routing
### Requests
When it comes to routing in Rocket, you need to use macros for your routes. Like many other features in Rocket, the usage of macros has propagated out to most other frameworks and is not otherwise possible.  Check out the following code below:
```rust
#[get("/")]
fn index() -> &'static str {
    "Hello, world!"
...
To be able to implement your own request guards, your type must implement the `FromRequest` trait. Let's have a look at how to implement the `FromRequest` trait. Like in other frameworks, you need to implem:
```rust
...
Instead of implementing a trait for your errors, error handling is done a bit differently. Instead of implementing an error handling trait, you handle errors by using an error handler which is called a "catcher" in Rocket. You can use a handler function for creating a catcher, like so:
```rust
#[catch(default)]
...
## Adding a Database
Normally when setting up a database, you might need to set up your own database connection. To get started with doing this in Rocket, you will need to add the `rocket-db-pools` crate with the `sqlx_postgres` feature:
```bash
...
```
You would then need to provision your own Postgres instance, whether installed locally on your computer, provisioned through Docker or something else, add it to your Rocket.toml file and then it works. Doing it this way means you can add it as a Request guard and it will also work:
```rust
#[get("/<id>")]
...
## App State
You can add application state like so:
```rust
use sqlx::PgPool;
...
Now you can use whatever's in your state struct this like:
```rust
...
## Fairings (Middleware)
Middleware in Rocket, or "fairings" as the crate itself calls them, are a way to add processes that take place before the request handler itself with the most common use cases being validation, authentication/authorization or rewriting request information before passing it onto another request. However, because fairings in Rocket affect the whole application rather than a set of routes, it is highly advised that you instead implement authentication and similar things that only need to be implemented over a certain number of routes as a Request guard rather than a fairing.

To write a fairing, you need to declare a struct that implements the `Fairing` trait. In terms of trait bounds, the type itself is required to be `Send + Sync + 'static` - meaning that essentially it just needs to be thread-safe and have only static references if any exist. See the code below:
```rust
use std::io::Cursor;
...
## Finishing Up
Thanks for reading! Although Rocket has previously fell out of favor among people who wanted to use cutting-edge Rust frameworks, the 0.5 upgrade brings a lot of new changes that will hopefully get it into the spotlight. 
