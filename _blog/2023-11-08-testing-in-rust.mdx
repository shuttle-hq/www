...
Although we will primarily be focusing on the Rust standard testing capabiilities for ease of use, there are also quite a few helper libraries for testing; for example, `test-case` which helps you build up test cases for tests by providing macros that you can stack on top of a test function and `lets_expect` which provides a macro in the form of `lets_expect!()` to help you test faster and more ergonomically. 
...
For example, if you have a file called `sum.js` with the following:
```javascript
// sum.js
...
## Finishing Up
Thanks for reading this article! I hope you have gained a better understanding of how testing in Rust works versus JavaScript and maybe a little bit of knowledge about setting up a basic web router in Rust using Axum.
...
- New to Rust? Check out [Shuttle Launchpad](https://www.shuttle.rs/launchpad), our free newsletter for learning Rust!
