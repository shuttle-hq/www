---
title: "Getting Started with Axum - Rust's Most Popular Framework"
...
With so many backend web frameworks in the Rust web ecosystem, it's difficult to know what to choose. Although much further in the past you might have seen Rocket In this article, we are looking at Axum, a web framework backed by the Tokio team that's simple to use and has hyper-compatibility with Tower, a robust library of reusable, modular components for building network applications. 

In this article we'll take a comprehensive look at writing an Axum web service. This will also include the [0.7 changes.](https://github.com/tokio-rs/axum/releases/tag/axum-v0.7.0)
...
Of course, we can also use a Result type for returns! Although the error type will also technically accept anything that can be turned into a HTTP response similarly to a successful request, we can also implement an error type that can illustrate several different ways a HTTP request can fail within our application just like we did with our successful HTTP request enum. See below:
```rust
enum ApiError {
    BadRequest,
    Forbidden,
...
This allows us to clearly differentiate between errors and successful requests when writing our Axum routing. 

## Adding a Database in Axum
Normally when setting up a database, you might need to set up your own database connection:
```rust
...
You would then need to provision your own Postgres instance, whether installed locally on your computer, provisioned through Docker or something else. However, with Shuttle we can eliminate this entirely as the runtime provisions the database for you:
...
```
It should also be noted that instead of using `#[derive(Clone)]`, you can also wrap the app state struct in an Atomic Reference Counter (`std::sync::Arc` for reference). Arcs are a form of garbage collection that keep track of how many clones there are and will only drop when there are no more copies left - and are a good type to know in Rust. This would be done like so:
```rust
use std::sync::Arc;
...
```
Note that any fields which are not in the struct <strong>will be ignored</strong> - depending on your use case, this can be a good thing; for example, if you're receiving a webhook but only want to look at certain fields from the webhook request. 
...
use axum_extra::TypedHeader; // use this if on axum 0.7
...
## Middleware in Axum
As mentioned before, one of Axum's great wins over other frameworks is that it is hyper-compatible with the `tower` crates, which means that we can effectively use any Tower middleware that we want with Axum! For example, we can add a Tower middleware to compress responses:
...
## Finishing Up
Thanks for reading! Axum is a great framework that has a strong team backing and is highly compatible with the Rust web ecosystem. 
