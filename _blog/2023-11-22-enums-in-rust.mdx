...
First, a quick recap of what enums actually are for the uninitiated (or those who need a reminder!) Enums are types that are able to represent a defined number of variants. Consider the following enum:
```rust
enum Directions {
  Up,
  Down,
...
For some context, let's have a look at what enums look like in other languages. In TypeScript, a cursory Google search for enums will return a number of results that either tell you the following:
...
## Finishing up
Thank you for reading! As you can see, enums in Rust are extremely powerful and form part of a strong backbone for Rust development. 
