...
We have changed our domain name to shuttle.dev! The new platform is available to use via the [shuttle.dev](http://Shuttle.dev) console. However, if you're an existing user looking to access the old platform and navigate between the two, you can find instructions [here](https://docs.shuttle.dev/introduction/platform-update).
...
### Looking to to migrate your project?
...
Get started now and head to [shuttle.dev](http://shuttle.dev) to join us redefining backend development.